...
license: MIT
compatibility: "Trae Skills / npx skills; instruction-only, no Codex hooks or agents."
---

# PUA for Trae — 高能动性治理 Skill

这个 Trae 版只用 `SKILL.md` 表达行为合约：Trae 可以加载 Skill，但不会自动获得 Codex 的 hooks、slash commands、subagents 和 Stop feedback。因此这里把治理边界写成**机械可执行的工作规程**，而不是靠一句“努力点”。
...
Trae 没有 Codex 的多 agent hook 编排时，也必须按下面的**权责边界**执行：
